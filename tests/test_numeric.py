import unittest
from bigquery_advanced_utils.utils import Numeric
...
class TestNumeric(unittest.TestCase):
    ...
    def test_convert_bytes_to_unit_invalid_unit(self) -> None:
        with self.assertRaises(ValueError) as context:
            self.numeric.convert_bytes_to_unit(1024, "INVALID")
    ...
    def test_convert_bytes_to_unit_negative_bytes(self):
        result = self.numeric.convert_bytes_to_unit(-1024, "KB")
        self.assertEqual(
            result, -1.0, "Negative byte values should be supported"
        )

    def test_convert_bytes_to_unit_zero_bytes(self):
        result = self.numeric.convert_bytes_to_unit(0, "KB")
        self.assertEqual(
